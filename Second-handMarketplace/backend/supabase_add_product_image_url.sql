-- Add a single editable image URL for products.
-- Run this in Supabase SQL Editor before using image_url from the frontend/API.
...
CREATE INDEX IF NOT EXISTS idx_categories_image_url
  ON public.categories(image_url)
  WHERE image_url IS NOT NULL AND image_url <> '';

-- Refresh search_products so RPC results also include image_url.
...
BEGIN
  ...
  SELECT
    ...
    p.image_url::TEXT, p.location::TEXT, p.status::TEXT,
    p.view_count::INTEGER, p.comment_count::INTEGER,
    p.avg_rating::NUMERIC, p.rating_count::INTEGER, p.is_negotiable::BOOLEAN,
    p.sold_at::TIMESTAMPTZ, p.created_at::TIMESTAMPTZ, p.updated_at::TIMESTAMPTZ,

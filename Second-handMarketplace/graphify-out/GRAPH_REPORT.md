...
## Corpus Check
- 212 files · ~90,747 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1398 nodes · 2588 edges · 96 communities (87 shown, 9 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 183 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d1e46ba0`
...
1. `useAuthStore` - 33 edges
2. `Query` - 20 edges
3. `ChatPage()` - 17 edges
4. `ProductDetailPage()` - 16 edges
5. `buildServiceError()` - 14 edges
6. `createNotification()` - 14 edges
7. `loadWithMocks()` - 14 edges
8. `AdminDashboardPage()` - 14 edges
9. `NotificationsPage()` - 14 edges
10. `requireAuth()` - 13 edges
...
Cohesion: 0.06
Nodes (65): createPaymentHandler(), {
  expireUnpaidTransactions,
  markTransactionPaymentCreated,
  prepareTransactionPayment,
}, getClientIp(), getIpnResponse(), { getPayment, updatePaymentFromGateway, upsertPayment }, getRequestBaseUrl(), handleVerifiedResult(), normalizePaymentPayload() (+57 more)
...
Cohesion: 0.12
Nodes (35): AuthLayout(), PasswordInput(), isSupabaseConfigured, ChangePasswordPage(), defaultForm, defaultForm, ForgotPasswordPage(), defaultForm (+27 more)
...
Nodes (39): createUserHandler(), {
  ...
  updateUserStatus,
  createUser,
}, getAdminOverviewHandler(), getAdminProductsHandler(), getAdminTransactionsHandler(), getAdminUsersHandler(), sendError(), updateProductStatusByAdminHandler() (+31 more)
...
Cohesion: 0.10
Nodes (21): autoprefixer, eslint-plugin-react, eslint-plugin-react-hooks, devDependencies, autoprefixer, eslint, eslint-config-prettier, eslint-plugin-react (+13 more)
...
Cohesion: 0.25
Nodes (7): author, description, keywords, license, name, type, version

### Community 35 - "AiSupportWidget.jsx"
Cohesion: 0.33
Nodes (4): AppError, AppError, formatIssues(), validateRequest()
...
Cohesion: 0.12
Nodes (25): ModerationPage(), STATUS_LABELS, CONDITION_LABELS, getProductImages(), PAYMENT_METHODS, ProductDetailPage(), readProductCache(), writeProductCache() (+17 more)

### Community 73 - "AiSupportWidget.jsx"
Cohesion: 0.18
Nodes (7): moderateReport, assert, { createReport, moderateReport }, fs, path, { resolveTarget }, test
...
Cohesion: 0.17
Nodes (14): SellerDashboard(), statusLabels, formatCurrency(), formatDate(), PAYMENT_STATUS_LABELS, PAYMENT_STATUS_STYLES, STATUS_LABELS, STATUS_STYLES (+6 more)
...
Cohesion: 0.25
Nodes (7): { createReport }, { createReportHandler, getMyReportsHandler }, express, { requireAuth }, router, validateRequest, createReport
...
Cohesion: 0.14
Nodes (19): App(), AdminRoute(), AgentRoute(), AuthOnlyRoute(), ProtectedRoute(), RootRedirect(), Navbar(), AgentInboxPage() (+11 more)
...
## Knowledge Gaps
- **545 isolated node(s):** `semi`, `singleQuote`, `trailingComma`, `printWidth`, `tabWidth` (+540 more)
...
- **Why does `ProductDetailPage()` connect `authorizationFlows.test.js` to `ClientHomePage.jsx`, `errorHandling.test.js`, `MyProductsPage.jsx`, `frontend/src/services/reviewService.js`, `tailwind-merge`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `TransactionHistoryPage()` connect `ChatPage.jsx` to `transactionFlows.test.js`, `errorHandling.test.js`, `frontend/src/services/reviewService.js`, `tailwind-merge`, `realtime.js`?**
